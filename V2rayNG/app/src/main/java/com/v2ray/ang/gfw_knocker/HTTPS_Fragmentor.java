	...
	public void run() {
		try{
			...
			while(true){					
					...
					My_upstream1 up_thread = new My_upstream1(client_sock , target_ip , target_port , DoH_obj , isFragment , num_fragment , fragment_sleep);
					up_thread.start();													
			}						
...
class My_upstream1 extends Thread{
	InputStream is;
	OutputStream os;
	Socket client_sock;
	Socket backend_sock;
	...
	public My_upstream1( Socket client_sock1 ,
						String target_ip1 , int target_port1, 
						DoH_over_Fragment DoH_obj1 ,  boolean isFragment1 , 
						int num_fragment1 , double fragment_sleep1 ){
	...
	public void run() {
		...
		try{
			...
			My_downstream1 down_thread = new My_downstream1( backend_sock.getInputStream() , client_sock.getOutputStream() );
			down_thread.start();
...
class My_downstream1 extends Thread{
	InputStream is;
	OutputStream os;
	byte[] buff ;
	int b;
	...
	public My_downstream1(InputStream backend_instream , OutputStream client_outstream){
		is = backend_instream;
		os = client_outstream;
		buff = new byte[4096];
	}
